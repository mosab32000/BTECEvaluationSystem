...
class AIEvaluator:
    ...
    def evaluate(self, task_submission):
        ...
        try:
            ...
            # Import openai here to ensure we're using the correct module
            from openai import OpenAI
            
            # Get API key from environment or config
            api_key = os.environ.get('OPENAI_API_KEY') or self.api_key
            
            # Create a client without any additional parameters
            ...
            logging.error(f"OpenAI module import error: {e}")
            return f"Error importing OpenAI module: {str(e)}"
    ...
    def evaluate_with_json(self, task_submission):
        ...
        try:
            ...
            # Create client without any additional parameters
            client = OpenAI(api_key=api_key)
            start_time = time.time()
            ...
            logging.error(f"OpenAI module import error in JSON evaluation: {e}")
            return {
                "grade": "Error",
                "feedback": [f"Error importing OpenAI module: {str(e)}"],
                "improvement_areas": ["Check OpenAI package installation"],
                "criteria_met": {
                    "knowledge": 0,
                    "application": 0,
                    "analysis": 0,
    ...
    def evaluate_with_rubric(self, task_submission, rubric=None):
        ...
        try:
            ...
            logging.error(f"OpenAI module import error in rubric evaluation: {e}")
            return {
                "grade": "Error",
                "total_score": 0,
                "sections": [],
                "overall_feedback": f"Error importing OpenAI module: {str(e)}",
                "error": True
            }
        except Exception as e:
